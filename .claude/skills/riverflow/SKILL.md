...
**Email sender overrides**: The base email sender wizard's `update_write_values` checks one bare context key. The **rivercreds** module extends the wizard: `_compute_attachment_ids` reads an additional bare key for credential auto-attach (see [rivercreds](../rivercreds/SKILL.md) Technical Reference). The **crewradar_sign** module extends the wizard: `_compute_attachment_ids` reads `chained_attachment_ids` for fill-then-email chaining (see [crewradar-sign — Fill-then-Email Chaining](../crewradar-sign/SKILL.md#fill-then-email-chaining)).
...
is tracked in [Golden Rule & Renewal Waivers §10/R6](../rivercreds/references/work-permit-golden-rules.md#10-related-requirements-surfaced-this-session-coverage-audit).
...
See [Radar & State Records](references/radar-state-records.md) for the full architecture, the `_for_record` override pattern, and how to add new models to the Radar.
...
**Which templates get which rule:**

- **Bring to Friday** -- everything that needs to be done before a date: book flights, arrange transport, send confirmations, AUV/A1/MFNL, timesheets, TFT contracts, check permits, and child services under AB/SAB/Matroos (like "Arrange Transport", "Inform Client")
- **Delay to Monday** -- everything that comes after a date: Aftercare, Journey Invoicing, diploma check
- **Allow weekend** -- Taxi, Train, Hotel, appointment services (AB, SAB, Doctor), quotes, sales
...
1. **Every field a rule's domain reads must also be in the trigger's `@api.depends`.** Otherwise the rule is evaluated once and never again: a record created with the field empty raises no service, someone fills the field the next day, and no service ever appears. Nothing reports the absence. `crewradar.log.entry._compute_service_ids` gained `employee_nationality_code` for exactly this reason. The same convention is written down for credential planning — see [credential-planning](../rivercreds/references/credential-planning.md), *"when adding a new domain that uses a common stored field, add the corresponding depends path"*.
...
**Test the domain STRING, not only the behaviour.** A domain lives in a `noupdate="1"` data file, so it is easy to change and hard to notice. On 2026-08-13 a fourth leaf was added to the MFNL sign-on rule that would have suppressed 7 legally owed government filings, and no test asserted the domain, so it landed unnoticed for twelve days. `crewradar_cuneus_sign/tests/test_mfnl_all_ships.py` now reads the record and asserts its leaves. Do the same for any rule whose *absence* of a service is a business risk.
...
**Caller's job — rehoming live services**: archiving a state does not touch `riverflow.service` rows that point at it. When a migration removes a state from XML, the per-workflow post-migrate must re-home any in-flight services to a surviving state *before* calling `reset_workflow_to_xml()` (the helper cannot know the correct target — it's workflow-specific). The full reshape recipe: (1) remove the dropped state/transition `<record>`s from the XML, (2) in post-migrate re-home live services off the dying states to a survivor, (3) **then** call `workflow.reset_workflow_to_xml()` (re-imports field values with `noupdate=False` AND archives the now-orphan records). Always re-home **before** reset, so no service still points at an about-to-be-archived state. See [crewradar_cuneus_sign/migrations/19.0.4.25/post-migrate.py](../../crewradar_cuneus_sign/migrations/19.0.4.25/post-migrate.py) for the canonical pattern (rehome Pickup Requested / Pickup Planned services to AT Applied, then reset). The Work Permit V2 reshape (migration 19.0.5.19) repeats it — re-home Checking-Permits services → Not Started after dropping `state_wp_check_permits`, then reset and seed the weekly service; 19.0.5.21 re-homes `state_weekly_bulk_upload` services → Permits Picked up after the bulk-upload state was demoted to a transition.

**Cross-workflow re-home (splitting one service off a shared workflow into its own):** when a service type moves onto a **new parallel** workflow while the old states stay alive for other consumers, the migration maps each in-flight instance's `state_id` from the old workflow's states to the equivalent new-workflow states. Writing **`state_id` alone is enough — `riverflow.state.mixin.write` auto-syncs `workflow_id` from `new_state.workflow_id`** (`_sync_workflow_with_state`), so a cross-workflow re-home needs no explicit `workflow_id` write. Discriminate the instances to move by a stable marker (a service tag), not shared-workflow membership, so siblings that legitimately stay on the shared workflow are left untouched. See [crewradar/migrations/19.0.9.158/post-migrate.py](../../crewradar/migrations/19.0.9.158/post-migrate.py) (TFT Contract split off the shared "Arrange and Send Document" workflow onto `workflow_tft_contract`, keyed by the `service_tag_tft` tag so the MFNL / A1-Application services on the same shared workflow are excluded). The template that seeds new instances is itself `noupdate="1"`, so the same migration force-writes its `state_id` + `days_relative_to_project` (the XML change is skipped on `-u`).
...
Full behaviour documentation (scales, data-horizon clamping, focus-date retention, sticky-left pinning) lives with the main consumer: [credential-planning.md — Phase 8A](../rivercreds/references/credential-planning.md#phase-8a--shared-timeline-utilities-in-riverflow) and the timeline frontend sections around it.
...
- [Riverflow bot execute (BUILT)](plans/riverflow-bot-execute.md) - part 2 of the bot-executes-transitions build; parts 1 and 3 are linked in its header
- [Host module Talents](../oteny-bot/plans/host-module-talents.md) - git path delivery for both host bundles
...
- [Credential Search Terms Plan](../rivercreds/plans/credential-search-terms.md) - plan-of-record (not yet built) for a `rivercreds` XML inherit of `state_record_view_search` that matches credential numbers; no line in `riverflow` itself changes
